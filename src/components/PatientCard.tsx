...
export function PatientCard({ patient, onUpdate, onDelete, onUndelete, selectionMode = false, isSelected = false, onToggleSelection, onTransfer, onPrintPatient }: PatientCardProps) {
  ...
  const removeArrayItem = (field: "diagnoses" | "medicalHistory" | "relevantExams" | "pendencies" | "utiAdmissionDate" | "utiDischargePrediction" | "utiAllergies" | "utiAdmissionReason" | "utiCurrentStatus" | "utiDevices" | "utiSpecialties" | "utiCulturesAntibiotics", index: number) => {
    const updatedPatient = { ...patient };
                ...
                <div className="flex flex-col md:col-span-3">
                  ...
                  <DndContext
                    ...
                    onDragEnd={(event: DragEndEvent) => {
                      const { active, over } = event;
                      if (over && active.id !== over.id) {
                        const oldIndex = (patient.utiAdmissionDate || []).findIndex((_, i) => `uti-admission-${i}` === active.id);
                        const newIndex = (patient.utiAdmissionDate || []).findIndex((_, i) => `uti-admission-${i}` === over.id);
                  ...
                  >
                    <SortableContext
                      items={(patient.utiAdmissionDate || []).map((_, i) => `uti-admission-${i}`)}
